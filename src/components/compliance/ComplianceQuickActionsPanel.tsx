import { ChevronRight, FilePlus2, UploadCloud, FileBarChart, BellRing } from "lucide-react";
import { ComplianceQuickAction } from "@/types/compliance";

interface ComplianceQuickActionsPanelProps {
  actions: ComplianceQuickAction[];
}

const ICONS: Record<ComplianceQuickAction["icon"], JSX.Element> = {
  add: <FilePlus2 size={17} />,
  upload: <UploadCloud size={17} />,
  report: <FileBarChart size={17} />,
  reminders: <BellRing size={17} />,
};

export function ComplianceQuickActionsPanel({ actions }: ComplianceQuickActionsPanelProps) {
  return (
